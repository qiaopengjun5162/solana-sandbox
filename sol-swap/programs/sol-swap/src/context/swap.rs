...
mod tests {
    ...
    #[test]
    fn test_swap_math() -> Result<()> {
       let balance_a: u64 = 25;
        let balance_b: u64 = 25;
        let amount = 4;
        let k = (balance_a as u128).checked_mul(balance_b.into()).ok_or(ProgramError::ArithmeticOverflow)?;
        assert_eq!(k, 625);
        let a2 = balance_a.checked_sub(amount).ok_or(ProgramError::ArithmeticOverflow)?;
        assert_eq!(a2, 21);
        let b2 = k.checked_div(a2.into()).ok_or(ProgramError::ArithmeticOverflow)?;
        println!("b2: {}", b2);
        assert_eq!(b2, 29);
        /*
        k = a * b
